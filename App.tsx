...
function AcademicCalendar({ onBack }:{ onBack:()=>void }) {
  ...
  return (
    <View style={s.flex}>
      <Header title="Academic Calendar" onBack={onBack}/>
      <View style={s.pagePad}><Segmented items={['Calendar View','Schedule Changes']} active={tab} onChange={setTab}/></View>
      <ScrollView contentContainerStyle={s.listPage} showsVerticalScrollIndicator={false}>
        {tab === 'Calendar View' ? (
          <>
            <View style={s.monthRow}><Ionicons name="chevron-back" size={20} color={C.navy}/><Text style={s.monthTitle}>October 2026</Text><Ionicons name="chevron-forward" size={20} color={C.navy}/></View>
            <View style={s.calendarCard}>
              <View style={s.weekRow}>{['Mon','Tue','Wed','Thu','Fri','Sat','Sun'].map(d => <Text key={d} style={s.weekText}>{d}</Text>)}</View>
              <View style={s.daysGrid}>{days.map(d => <View key={d} style={[s.dayCell, d===15 && s.dayOn]}><Text style={[s.dayText, d===15 && { color:C.white }]}>{d}</Text></View>)}</View>
            </View>
            <SectionTitle title="Upcoming" action="See All"/>
            {calendarItems.map(([date,title,type], i) => <ListCard key={title} icon={type==='Exam' ? 'document-text-outline' : 'calendar-outline'} title={title} subtitle={date} meta={type} red={i===0}/>) }
          </>
        ) : (
          <>
            <ListCard icon="alert-circle-outline" title="Room Change" subtitle="Software Engineering lecture moved from B203 to A101." meta="Today • 2:00 PM" red />
            <ListCard icon="time-outline" title="Class Time Updated" subtitle="Business tutorial now starts at 11:30 AM." meta="Tomorrow" />
            <ListCard icon="cloudy-outline" title="Campus Schedule Notice" subtitle="Official changes and closures will appear here when published." meta="University notice" />
          </>
        )}
      </ScrollView>
    </View>
...
function Programmes({ onBack }:{ onBack:()=>void }) {
  ...
  const filtered = useMemo(() => programmes.filter(([name, partner]) => `${name} ${partner}`.toLowerCase().includes(search.toLowerCase())), [search]);
  return (
    <View style={s.flex}>
      <Header title="Programmes" onBack={onBack}/>
      <ScrollView contentContainerStyle={s.listPage} showsVerticalScrollIndicator={false}>
        <View style={s.programmeHero}><Image source={require('./assets/programme-banner.png')} style={StyleSheet.absoluteFillObject} resizeMode="cover"/><View style={s.programmeShade}/><Text style={s.programmeHeroTitle}>World-class programmes{`\n`}for a global future.</Text></View>
        <View style={{ marginTop: 14 }}><SearchBox value={search} onChangeText={setSearch} placeholder="Search programmes..."/></View>
        <SectionTitle title="Explore categories" />
        {programmeGroups.map(p => <ListCard key={p.title} icon={p.icon as any} title={p.title} subtitle={p.subtitle} red />)}
        <SectionTitle title="Programme list" />
        {filtered.map(([name, partner]) => <ListCard key={name} icon="book-outline" title={name} subtitle={partner}/>) }
      </ScrollView>
    </View>
  );
}
